...
if(isset($_POST['submitted'])) {
    ...
    if(!isset($hasError)) {
        ...
        $subject = __('From ','_mh').$name;
        $body = __('Name: ','_mh').$name."\n".__('Email: ','_mh').$email."\n".__('Comments: ','_mh').$comments;
        $headers = __('From: ','_mh') .$name. ' <'.$emailTo.'>' . "\r\n" . __('Reply-To:','_mh') .$name. '<'.$email.'>';
						...
						<?php the_content(); ?>
				          ...
				          <?php if(isset($emailSent) && $emailSent == true) { ?>
				                <div class="alert alert-success" role="alert">
				                    <p><?php _e('Thanks, your email was sent successfully.', '_mh'); ?></p>
				                </div>
				            <?php } else { ?>
				                ...
				                <?php if(isset($hasError) || isset($captchaError)) { ?>
				                    <div class="alert alert-danger alert-dismissible" role="alert">
									  ...
									  <strong><?php _e('Error!', '_mh'); ?></strong> <?php _e('Please try again.', '_mh'); ?>
									</div>
				                <?php } ?>

						        <form action="<?php the_permalink(); ?>" id="contactForm" method="post">
						            <div class="form-group <?php if(isset($nameError)) { echo "has-error has-feedback"; }?>">
						                    <label class="control-label" for="contactName"><?php _e('Name', '_mh'); ?></label>
						               ...
						               <div class="form-group <?php if(isset($emailError)) { echo "has-error has-feedback"; }?>">
						                    <label class="control-label" for="email"><?php _e('Email', '_mh'); ?></label>
						                ...
						                <div class="form-group <?php if(isset($commentError)) { echo "has-error has-feedback"; }?>">
						                    <label class="control-label" for="commentsText"><?php _e('Message', '_mh'); ?></label>
						               ...
						               <div class="form-actions">
						                    <button type="submit" class="btn btn-primary"><?php _e('Send Email', '_mh'); ?></button>
